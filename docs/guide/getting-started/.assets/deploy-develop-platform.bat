...
set MYSQL_DB_IMAGE_NAME=mysql/mysql-server:8.0
set MYSQL_DB_HOST_PATH="mysql-data"

rem rabbitmq 相关参数
set RABBITMQ_IMAGE_NAME=rabbitmq:3-management
...
rem 镜像名称
set MOTHERSHIP_IMAGE_NAME=whxinpai/ouroboros-mothership:!OUROBOROS_VERSION!
set DEVELOP_IMAGE_NAME=whxinpai/ouroboros-develop:!OUROBOROS_VERSION!
...
:config_rabbitmq
  ...
  call :_resetReturn
  for /f "delims=" %%t in ('docker ps -a --filter "ancestor=rabbitmq:3-management" --filter "name=ouroboros-rabbitmq" --format "{{.ID}}"') do set return=%%t
  if "!return!" equ ""  (
  ...
  call :_resetReturn
  for /f "delims=" %%t in ('docker ps -a --filter "ancestor=rabbitmq:3-management" --filter "name=ouroboros-rabbitmq" --filter "status=exited" --format "{{.ID}}"') do set return=%%t
  if "!return!" neq ""  (
...
rem 拉取开发环境镜像
:pull_develop
  call :__pull !DEVELOP_IMAGE_NAME! !IS_MULTI_ARCH!
goto :eof

rem 拉取基座镜像
:pull_mothership
  call :__pull !MOTHERSHIP_IMAGE_NAME! !IS_MULTI_ARCH!
goto :eof
...
:__pull
  ...
  if "!is_multi_arch!" equ "true" (
    call :__get_architecture
    set arch=!return!
    
    docker pull --platform=linux/!arch! -q !image_name! >nul 2>&1
  ) else (
    docker pull -q !image_name! >nul 2>&1
  )
