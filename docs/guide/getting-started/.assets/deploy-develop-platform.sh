...
MYSQL_DB_PASSWORD=""
MYSQL_DB_PORT=13306
MYSQL_DB_IMAGE_NAME=mysql/mysql-server:8.0
...
# 镜像名称
MOTHERSHIP_IMAGE_NAME=whxinpai/ouroboros-mothership:$OUROBOROS_VERSION
DEVELOP_IMAGE_NAME=whxinpai/ouroboros-develop:$OUROBOROS_VERSION
...
config_rabbitmq() {
  ...
  if [ ! "$(docker ps -a --filter name="^ouroboros-rabbitmq$" --format {{.ID}})" ]; then
    ...
    docker run -d \
      ...
      ${RABBITMQ_IMAGE_NAME} >/dev/null 2>&1
...
init_dev_database_mysql() {
  ...
  docker run -dit \
    ...
    --network ${OUROBOROS_DEVELOP_NETWORK_NAME} \
    ${MYSQL_DB_IMAGE_NAME} >/dev/null 2>&1
...
recreate_develop_container() {
  ...
  docker run -dit \
    ...
    --network ${OUROBOROS_DEVELOP_NETWORK_NAME} \
    ${DEVELOP_IMAGE_NAME} >/dev/null 2>&1
...
__pull() {
  local image_name=$1
  local is_multi_arch=$2
  local pull_image_name=$(get_pull_image_name ${image_name} ${is_multi_arch})
  if [ "${IS_FROM_ALI}" == "true" ]; then
    local pull_result=$(docker pull -q ${ALI_REGISTRY_URL}/${pull_image_name})
    docker tag ${ALI_REGISTRY_URL}/${pull_image_name} ${image_name}
    ...
    local pull_result=$(docker pull -q ${image_name})
    if [ -z "${pull_result}" ]; then
      _error "拉取镜像(${image_name})失败" 2
      exit 1
    fi
...
# 拉取基座镜像
pull_mothership() {
  __pull ${MOTHERSHIP_IMAGE_NAME} ${IS_MULTI_ARCH}
}

# 拉取开发环境镜像
pull_develop() {
  __pull ${DEVELOP_IMAGE_NAME} ${IS_MULTI_ARCH}
}
...
get_pull_image_name() {
  local image_name=$1
  local is_multi_arch=$2
  local arch=$(__get_architecture)
  if [ "${is_multi_arch}" == "false" ]; then
    echo ${image_name}
    return
  fi
  echo "${image_name}/${OUROBOROS_VERSION}/${OUROBOROS_VERSION}-${arch}"
}
